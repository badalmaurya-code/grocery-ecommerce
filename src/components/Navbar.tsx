...
export const Navbar: React.FC<NavbarProps> = ({ currentView, navigate }) => {
            ...
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center gap-3 px-2 py-1.5">
                ...
                <div className="flex-1 space-y-1.5 min-w-0">
                  ...
                  <div className="h-2.5 bg-stone-200/60 rounded w-1/2" />
                </div>
                <div className="h-3.5 w-12 bg-stone-200 rounded shrink-0" />
              </div>
            ))}
          </div>
  ...
  return (
    <>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-stone-200/90 py-1.5 px-2 flex items-center justify-around shadow-2xl safe-area-bottom">
        <button
          onClick={() => navigate('home')}
          className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all cursor-pointer ${
            currentView === 'home' ? 'text-emerald-800 font-bold' : 'text-stone-500 hover:text-stone-900'
          }`}
        >
        ...
        <button
          onClick={() => navigate('shop')}
          className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all cursor-pointer ${
            currentView === 'shop' ? 'text-emerald-800 font-bold' : 'text-stone-500 hover:text-stone-900'
          }`}
        >
        ...
        <button
          onClick={() => navigate('categories')}
          className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all cursor-pointer ${
            currentView === 'categories' ? 'text-emerald-800 font-bold' : 'text-stone-500 hover:text-stone-900'
          }`}
        >
          <Grid className={`w-5 h-5 ${currentView === 'categories' ? 'stroke-[2.5]' : 'stroke-2'}`} />
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all cursor-pointer ${
            currentView === 'my-orders' || currentView === 'order-detail' ? 'text-emerald-800 font-bold' : 'text-stone-500 hover:text-stone-900'
          }`}
        >
          <Truck className={`w-5 h-5 ${currentView === 'my-orders' || currentView === 'order-detail' ? 'stroke-[2.5]' : 'stroke-2'}`} />
        ...
        <button
          ...
          className={`relative flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all cursor-pointer ${
            currentView === 'cart' ? 'text-emerald-800 font-bold' : 'text-stone-500 hover:text-stone-900'
          }`}
        >
          <div className="relative">
            <ShoppingBag className={`w-5 h-5 ${currentView === 'cart' ? 'stroke-[2.5]' : 'stroke-2'}`} />
            {totalItems > 0 && (
